...
namespace UPProjects.Services
{
    ...
    {
        AuthenticateResponse Authenticate(LoginViewModel model, string ipAddress);
    ...
    {        
        ...
        public AuthenticateResponse Authenticate(LoginViewModel model, string ipAddress)
        {
            var user = (dynamic)null;
            using (var connection = new SqlConnection(_connectionString))
            {
                connection.Open();
                 user= connection.QueryFirst<User>("GetLoginDetailByUserId", new { UserId = model.Email }, commandType: CommandType.StoredProcedure);
                connection.Close();
            }
            ...
            if (user.Password == model.Password)
            {
                ...
                if (insertRefreshTokens(refreshToken, model.Email))
        ...
        public AuthenticateResponse RefreshToken(string token, string ipAddress)
        {
            ...
            if (chkResult != null) { 
                if (chkResult.Userid != null && chkResult.Userid != "")
                {
                    ...
                    {
                        connection.Open();
                        user = connection.QueryFirst<User>("GetLoginDetailByUserId", new { UserId = chkResult.Userid }, commandType: CommandType.StoredProcedure);
                        connection.Close();
                    }
                    var jwtToken = generateJwtToken(user);
